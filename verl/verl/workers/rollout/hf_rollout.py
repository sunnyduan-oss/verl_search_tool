...
class HFRollout(BaseRollout):
    ...
    def chat_sequences(self, prompts):
        batch_size = len(prompts)
        print("batch_size: ", batch_size)
        # num_chunks = max(batch_size // self.config.get("micro_batch_size", batch_size), 1)
        # batch_prompts = prompts.chunk(chunks=num_chunks)
        # print("batch_prompts: ", batch_prompts)
        output = [self._chat_minibatch(p) for p in prompts]
    ...
    def _chat_minibatch(self, prompts) -> DataProto:
        # 取参数
        question = prompts[0]['question']
        # print("question: ", question)
        pixel_values = prompts[0]['pixel_values']
        num_patches_list = prompts[0]["num_patches_list"]
        ...
        with param_ctx, torch.autocast(device_type=get_device_name(), dtype=torch.bfloat16):
            response, history = self.module.chat(
                self.tokenizer, 
                pixel_values, 
                question, 
    ...
    def _generate_minibatch(self, prompts: DataProto) -> DataProto:
        ...
        self.module.img_context_token_id = prompts.meta_info.get("img_context_token_id")
        print("img_context_token_id: ", self.module.img_context_token_id)
